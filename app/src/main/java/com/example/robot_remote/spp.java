...
//    private static final int REQUEST_STORY_CODE = 3; // 文件读取权限
    ...
    public void onClick(View view) {
        ...
        {
            Intent intent = new Intent(spp.this,MainActivity.class);
...
//        builder.setScanMode(ScanSettings.SCAN_MODE_BALANCED); // 平衡模式，耗电适中，扫描时间间隔一般，我使用这种模式来更新设备状态
    ...
    private void tipGPSSetting() {
        AlertDialog.Builder builder = new AlertDialog.Builder(spp.this);
        builder.setTitle("提示");
        builder.setMessage("安卓6.0以后使用蓝牙需要开启定位功能，但本应用不会使用到您的位置信息，开始定位只是为了扫描到蓝牙设备。是否确定打开");
        builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                GpsUtil.openGPS(spp.this, SET_GPS_OPEN_STATE);
            }
        });
        ...
        builder.setNegativeButton("取消", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                ToastUtil.show(spp.this, "您无法使用此功能");
            }
        });
        builder.show();
    }
...
//                    ToastUtil.show(this, "错误1！");
                    ...
                    builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            launchAppDetailsSettings(spp.this);
                        }
                    });
...
//                                    LogUtil.i("设备已连接上，开始扫描服务");
                        ...
                        if (reConnectionNum < maxConnectionNum) {
                            ...
                            // 连接设备
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                mBluetoothGatt = mBluetoothDevice.connectGatt(spp.this,
                                        false, mGattCallback, BluetoothDevice.TRANSPORT_LE);
                            } else {
                                mBluetoothGatt = mBluetoothDevice.connectGatt(spp.this, false, mGattCallback);
                            }
                        } else {
                            // 断开连接，失败回调
                            ToastUtil.show(spp.this, "蓝牙连接失败，建议重启APP，或者重启蓝牙，或重启设备");
                            closeBLE();
                        }
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        //获取指定uuid的service
                        BluetoothGattService gattService = mBluetoothGatt.getService(UUID.fromString(UUDI_2));
//                        bluetoothGattServiceList.add(gattService);
                        ...
                        if (gattService != null) {
                            ...
                            gattCharacteristic =
                                    gattService.getCharacteristic(UUID.fromString(CHARACTERISTIC_UUID_2));
...
//                                    //通过Gatt对象读取特定特征（Characteristic）的特征值
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, final BluetoothGattCharacteristic characteristic) {
            ...
            control.receivedata_spp = DigitalTrans.bytetoString(value);
            Log.d("ceshi",control.receivedata_spp);
            try {
                control.spp_data_change();
            } catch (Exception e) {
